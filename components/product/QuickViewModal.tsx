...
export function QuickViewModal({
  ...
  return createPortal(
    ...
    >
      <div
        className="relative flex max-h-[90dvh] w-full max-w-3xl flex-col overflow-hidden rounded-t-2xl border border-stone-300 bg-white md:max-h-[90vh] md:flex-row md:rounded-none"
      ...
      >
        ...
        <div className="flex min-h-0 w-full flex-1 flex-col md:w-1/2">
          <div className="min-h-0 flex-1 overflow-y-auto p-3 md:p-8">
            ...
            <h2 className="mb-1 text-base font-semibold leading-snug text-[var(--foreground)] md:mb-2 md:text-2xl">
              {product.name}
            </h2>

            <div className="mb-3 flex items-baseline gap-2 md:mb-4 md:gap-3">
              <span className="text-lg font-semibold text-[var(--foreground)] md:text-2xl">
            ...
            {product.colors && product.colors.length > 0 ? (
              <div className="mb-3 md:mb-4">
                <label className="mb-1.5 block text-[10px] font-semibold uppercase tracking-wider text-[var(--foreground)] md:mb-2 md:text-xs">
            ...
            <div className="mb-4 flex items-center gap-3 md:mb-6 md:gap-4">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-[var(--foreground)] md:text-xs">
                Quantity
              </span>
              <div className="flex items-center border border-stone-200 bg-stone-50">
                <button
                  ...
                  className="px-3 py-1.5 font-bold text-stone-600 hover:bg-stone-200"
                >
                  -
                </button>
                <span className="min-w-10 px-4 py-1.5 text-center text-sm font-semibold text-stone-900">
                  {quantity}
                </span>
                <button
                  type="button"
